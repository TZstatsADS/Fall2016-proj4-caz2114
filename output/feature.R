...
#load data file
setwd("C:/Users/CATHY/OneDrive/Documents/2016-2017 Junior/03 Applied Data Science/Project 4/Fall2016-Proj4-caz/Data")
load("./lyr.RData")
load("./h5file.RData")
load("./h5_analysis.RData")
load("./fit.RData")



list_bars_confidence <- vector(mode = "list",length = 2350)
list_bars_start <- vector(mode = "list",length = 2350)
list_beats_confidence <- vector(mode = "list",length = 2350)
list_beats_start <- vector(mode = "list",length = 2350)
list_sections_confidence <- vector(mode = "list",length = 2350)
list_sections_start <- vector(mode = "list",length = 2350)
list_segments_confidence <- vector(mode = "list",length = 2350)
list_segments_loudness_max <- vector(mode = "list",length = 2350)
list_segments_loudness_max_time <- vector(mode = "list",length = 2350)
list_segments_loudness_start <- vector(mode = "list",length = 2350)
list_segments_pitches <- vector(mode = "list",length = 2350)
list_segments_start <- vector(mode = "list",length = 2350)
list_segments_timbre <- vector(mode = "list",length = 2350)
list_tatums_confidence <- vector(mode = "list",length = 2350)
list_tatums_start <- vector(mode = "list",length = 2350)
...
#created for handling less variable
smoothing_less <- function(data){
  y <- data
  if(length(y) != 0){
    x <- 1:length(y)
    result <- predict(smooth.spline(x,y,spar = 0.1),(length(y)/10)*1:10)
    result <- result$y
  }else {
    result <- rep.int(0,10)
  }
...
for( i in 1:length(h5file)){
  ...
  this <- unlist(h5file[[i]]$songs)
  h5_song[[i]] <- unname(as.numeric(this[!is.na(as.numeric(as.character(this)))]))
}
